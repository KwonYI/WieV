...
public class GroupAllController {
	...
	public ResponseEntity<List<Applicant>> getGroupApplicantList(@PathVariable("groupSeq") int groupSeq) {
		...
		try {
			...
			for (int i = 0; i < groupDetailList.size(); i++) {
				...
				tmpApplicantGroupList = applicantGroupDao
						.findListApplicantGroupByGroupDetailDetailSeq(nowGroupDetail.getDetailSeq());
	...
	@ApiOperation(value = "공고에 따른 면접 일정 전체 삭제하기")
			...
			if (!groupAllList.isEmpty()) {
				...
				for (int i = 0; i < groupAllList.size(); i++) {
					...
					for (int j = 0; j < groupTypeList.size(); j++) {
						...
						List<TypeInterviewer> typeInterviewerList = typeInterviewerDao.findListTypeInterviewerByGroupTypeGroupTypeSeq(groupType.getGroupTypeSeq());
				...
				System.out.println("공고에 따른 면접 일정 없음");
	...
	public ResponseEntity<List<Interviewer>> getWaitInterviewerList(@PathVariable("groupSeq") int groupSeq) {
		...
		try {
			...
			for (int i = 0; i < groupTypeList.size(); i++) {
				...
				tmpTypeInterviewerList = typeInterviewerDao
						.findAllTypeInterviewerByGroupTypeGroupTypeSeqAndGroupTypeInterviewTypeTypeSeq(
								nowGroupType.getGroupTypeSeq(), nowGroupType.getInterviewTypeTypeSeq());
	...
	public Object divide(@Valid @RequestBody GroupAllRequest groupAllRequest,
			@PathVariable("recruitSeq") int recruitSeq) {
		HttpStatus status = null;
...
//						System.out.println(groupAllRequest.getInterviewTypeList().get(j));
						...
						groupType.setInterviewTypeTypeSeq(interviewTypeDao
								.findInterviewTypeByTypeName(groupAllRequest.getInterviewTypeList().get(j))
								.getTypeSeq());
