...
public class JwtService {
	...
	public String create(Hr memberDto) {
		...
		jwtBuilder
			...
			.claim("user", memberDto).claim("greeting", "환영합니다. " + memberDto.getHrEmail()); // 담고 싶은 정보 설정.
	...
	public String create(Interviewer memberDto) {
		JwtBuilder jwtBuilder = Jwts.builder();
		//JWT Token = Header + Payload + Signature
		...
		jwtBuilder
			.setSubject("로그인토큰") // 토큰의 제목 설정
			.setExpiration(new Date(System.currentTimeMillis() + 1000 * 60 * expireMin)) // 유효기간 설정
			.claim("user", memberDto).claim("greeting", "환영합니다. " + memberDto.getViewEmail()); // 담고 싶은 정보 설정.
