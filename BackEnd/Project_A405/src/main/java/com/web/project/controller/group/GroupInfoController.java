...
public class GroupInfoController {
	...
	public ResponseEntity<List<Map<String, Object>>> totalList(@PathVariable("recruitSeq") int recruitSeq) {
...
//				JSONObject jsonObject = new JSONObject();
				...
				// 3. 그룹 Seq
				resultMap.put("groupSeq", groupAll.getGroupSeq());

				// 4. 그룹 면접 실시 날짜
				resultMap.put("groupDate", groupAll.getGroupDate());

				// 5. 그룹 면접 시작 시간
				resultMap.put("groupStartTime", groupAll.getGroupStartTime());
				...
				resultMap.put("interviewTypeList", interviewTypeList);
				...
				resultMap.put("waitInterviewerList", waitInterviewerList);
				resultMap.put("interviewerList", interviewerList);
				...
				resultMap.put("groupApplicantList", groupApplicantList);
				resultMap.put("groupDetailList", groupDetailList);

				// 10. 해당 그룹이 어떤 직무인지 저장
				resultMap.put("groupCareerName", careerDao.findCareerByCaSeq(groupAll.getCareerCaSeq()).getCaName());

				// 11. JSON Array에 넣는다.
				groupInfoList.add(resultMap);
		...
		return new ResponseEntity<List<Map<String, Object>>>(groupInfoList, status);
	}

	@GetMapping("/comTotalGroupList/{comSeq}")
	@ApiOperation(value = "회사의 공고 그룹 모두 가져오기")
	public ResponseEntity<List<Map<String, Object>>> comTotalGroupList(@PathVariable("comSeq") int comSeq) {
		HttpStatus status = null;
//		JSONArray jsonArray = new JSONArray();
		List<Map<String, Object>> comGroupInfoList = new ArrayList<Map<String, Object>>();

		try {
			List<Recruit> recruitList = recruitDao.findAll();

			for (int c = 0; c < recruitList.size(); c++) {
				if (recruitList.get(c).getCompanyComSeq() == comSeq) {
					Map<String, Object> groupInfoMap = new HashMap<String, Object>();
					groupInfoMap.put("recruitSeq", recruitList.get(c).getReSeq());
					List<Map<String, Object>> groupInfoList = new ArrayList<Map<String, Object>>();
					List<GroupAll> groupAllList = groupAllDao.findListGroupAllByRecruitReSeq(recruitList.get(c).getReSeq());
...
//						JSONObject jsonObject = new JSONObject();
						...
						for (int j = 0; j < groupTypeList.size(); j++) {
							interviewTypeList.add(interviewTypeDao
									.findInterviewTypeByTypeSeq(groupTypeList.get(j).getInterviewTypeTypeSeq())
									.getTypeName());
						}
						resultMap.put("interviewTypeList", interviewTypeList);
						...
						for (int j = 0; j < groupTypeList.size(); j++) {
							List<TypeInterviewer> typeInterviewerList = typeInterviewerDao
									.findListTypeInterviewerByGroupTypeGroupTypeSeq(
											groupTypeList.get(j).getGroupTypeSeq());
							...
							for (int k = 0; k < typeInterviewerList.size(); k++) {
								...
								interviewerMap.put("interviewType",
										interviewTypeDao
												.findInterviewTypeByTypeSeq(
														typeInterviewer.getGroupTypeInterviewTypeTypeSeq())
												.getTypeName());
						...
						for (int j = 0; j < detailList.size(); j++) {
							...
							for (int k = 0; k < detailOrderList.size(); k++) {
								detailOrder.add(interviewTypeDao
										.findInterviewTypeByTypeSeq(
												detailOrderList.get(k).getGroupTypeInterviewTypeTypeSeq())
										.getTypeName());
							}
							...
							for (int k = 0; k < applicantGroupList.size(); k++) {
								...
								String name = applicantDao
										.findApplicantByApplySeq(applicantGroup.getApplicantApplySeq()).getApplyName();
						...
						// 10. 해당 그룹이 어떤 직무인지 저장
						resultMap.put("groupCareerName",
								careerDao.findCareerByCaSeq(groupAll.getCareerCaSeq()).getCaName());
...
//					System.out.println(groupInfoList);
					groupInfoMap.put("groupInfoList", groupInfoList);
					comGroupInfoList.add(groupInfoMap);
				}
			}

			status = HttpStatus.OK;
		} catch (RuntimeException e) {
			comGroupInfoList = null;
			logger.error("공고에 해당하는 모든 정보 가져오기 실패", e);
			status = HttpStatus.INTERNAL_SERVER_ERROR;
		}

		return new ResponseEntity<List<Map<String, Object>>>(comGroupInfoList, status);
	}
}
