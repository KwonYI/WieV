...
import com.web.project.model.hr.FindPasswordRequest;
...
//	}
	...
	public ResponseEntity<Map<String, Object>> findPassword(@RequestBody FindPasswordRequest findPasswordRequest) throws MessagingException {
		HttpStatus status = null;
		Map<String, Object> resultMap = new HashMap<>();
		
		try {
			System.out.println(findPasswordRequest.getHrEmail()+" "+findPasswordRequest.getHrName()+" "+findPasswordRequest.getHrPhone());
			Optional<Hr> hrOpt = hrDao.findHrByHrEmailAndHrNameAndHrPhone(findPasswordRequest.getHrEmail(),findPasswordRequest.getHrName(),findPasswordRequest.getHrPhone());
			String password=getUUID().substring(0, 10)+"!";
			
			if (hrOpt.isPresent()) {
				findPasswordMail(findPasswordRequest.getHrEmail(),password);
				
				hrOpt.ifPresent(selectHr -> {
	            	selectHr.setHrPassword(password);
	            	hrDao.save(selectHr);
	            });
...
//	}
	...
	@ApiOperation(value = "인증 메일 보내기")
	public void findPasswordMail(String email,String password) throws MessagingException {
...
//		String certifiedKey=certified_key();
		...
		emailContent.append("  <table style=\"border-collapse: collapse;table-layout: fixed;border-spacing: 0;mso-table-lspace: 0pt;mso-table-rspace: 0pt;vertical-align: top;min-width: 320px;Margin: 0 auto;background-color: #fbfbfb;width:100%\" cellpadding=\"0\" cellspacing=\"0\">\r\n" + 
				...
				+"</span></span><span style=\"font-size: 22px; line-height: 30.8px; color: #3b7df0;\">새로운 비밀번호</span>"
				...
				"  <p style=\"font-size: 14px; line-height: 190%;\">WieV의 새로운 비밀번호입니다.<br />아래<strong><span style=\"color: #363d5a; font-size: 14px; line-height: 26.6px;\\\">'비밀번호'</span></strong>로 로그인을 진행해주세요.<br />감사합니다.</p>\r\n" +
				"  <br/><strong><span style=\"color: #363d5a; font-size: 14px; line-height: 26.6px;\">"+password+"</span></strong>" + 
				"  </div>\r\n" + 
				"\r\n" + 
				"      </td>\r\n" + 
				"    </tr>\r\n" + 
